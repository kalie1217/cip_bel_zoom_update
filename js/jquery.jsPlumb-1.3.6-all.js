...
(function () {
    ...
    _indexOf = function (l, v) {
        return _findWithFunction(l, function (_v) {
            return _v == v;
        });
    },
        ...
        this.fire = function (event, value, originalEvent) {
            if (_listeners[event]) {
                for (var i = 0; i < _listeners[event].length; i++) {
                    ...
                    if (_findWithFunction(eventsToDieOn, function (e) {
                        return e === event
                    }) != -1)
                    _listeners[event][i](value, originalEvent); else {
    ...
    jsPlumbUIComponent = function (params) {
        ...
        this.isDropAllowed = function (sourceId, targetId, scope) {
            var r = self._jsPlumb.checkCondition("beforeDrop", {
                sourceId: sourceId, targetId: targetId, scope: scope
            });
            if (beforeDrop) {
                try {
                    r = beforeDrop({
        ...
        this.setHover = function (hover, ignoreAttachedElements, timestamp) {
            ...
            if (! self._jsPlumb.currentlyDragging && ! self._jsPlumb.isHoverSuspended()) {
                ...
                if (self.hoverPaintStyle != null) {
                    ...
                    self.repaint({
                        timestamp: timestamp, recalc: false
                    });
                }
    ...
    overlayCapableJsPlumbUIComponent = function (params) {
        ...
        //params.overlays || self._jsPlumb.Defaults.Overlays;
        if (_overlays) {
    ...
    var jsPlumbInstance = function (_defaults) {
        ...
        _currentInstance.restoreDefaults = function () {
            _currentInstance.Defaults = jsPlumb.extend({
            },
            _initialDefaults);
        };
        ...
        _elementProxy = function (element, fn) {
            ...
            if (element. constructor == Array) {
                ...
                for (var i = 0; i < element.length; i++) {
                    ...
                    // append return values to what we will return
                }
            } else {
        ...
        _newConnection = function (params) {
            ...
            if (params.container)
            params[ "parent"] = params.container; else {
        ...
        _getParentFromParams = function (params) {
            ...
            return params.container; else {
                ...
                return jsPlumb.CurrentLibrary.getParent(p); else return p;
            }
        },
        ...
        _updateOffset = function (params) {
            ...
            if (recalc || ! offset) {
                // if forced repaint or no offset
        ...
        this.addEndpoint = function (el, params, referenceParams) {
            ...
            for (var i = 0; i < inputs.length; i++) {
                ...
                var anchorLoc = e.anchor.compute({
                    xy:[myOffset.left, myOffset.top], wh: myWH, element: e
                });
                e.paint({
                    anchorLoc: anchorLoc
                });
        ...
        this.detach = function () {
            ...
            params = arguments.length == 2 ? firstArgIsConnection ? (arguments[1] || {
            ...
            if (connection) {
                if (forceDetach || (connection.isDetachAllowed(connection) && connection.endpoints[0].isDetachAllowed(connection) && connection.endpoints[1].isDetachAllowed(connection))) {
                    ...
                    // TODO check this param iscorrect for endpoint's detach method
                }
            } else {
                var _p = jsPlumb.extend({
                },
                params);
                // a backwards compatibility hack: source should be thought of as 'params' in this case.
        ...
        this.init = function () {
            if (! initialized) {
                ...
                // calling the method forces the capability logic to be run.
        ...
        this.makeAnchors = function (types, elementId, jsPlumbInstance) {
            ...
            for (var i = 0; i < types.length; i++) {
                ...
                r.push(jsPlumb.Anchors[types[i]]({
                    elementId: elementId, jsPlumbInstance: jsPlumbInstance
        ...
        this.makeTarget = function (el, params, referenceParams) {
            ...
            _doOne = function (_el) {
                ...
                var dropOptions = jsPlumb.extend({
                },
                p.dropOptions || {
                }),
                _drop = function () {
                    ...
                    _endpoint = p.endpoint ? jsPlumb.extend({
                    },
                    p.endpoint): {
                    };
                    ...
                    // if not allowed to drop...
                        ...
                        if (jpc.suspendedEndpoint) {
                            ...
                            source.detach(jpc, false, true, true);
                            // otherwise, detach the connection and tell everyone about it.
                        }
                    }
                };
        ...
        this.makeSource = function (el, params, referenceParams) {
            ...
            _doOne = function (_el) {
                ...
                dragOptions[stopEvent] = function () {
                    ...
                    _currentInstance.deleteEndpoint(ep); else {
        ...
        this.repaint = function (el) {
            ...
            // support both lists...
            if (typeof el == 'object')
            for (var i = 0; i < el.length; i++) _processElement(el[i]); else // ...and single strings.
            _processElement(el);
        };
        ...
        this.registerListener = function (el, type, listener) {
            ...
            _addToList(_registeredListeners, type, {
                el: el, event: type, listener: listener
            });
        };
        ...
        this.setRenderMode = function (mode) {
            ...
            renderMode = jsPlumb.CANVAS; else if (mode === jsPlumb.SVG && svgAvailable)
            renderMode = jsPlumb.SVG; else if (vmlAvailable)
        ...
        calculateOrientation = function (sourceId, targetId, sd, td) {
            ...
            if (! (h || v)) {
                ...
                return {
                    orientation: Orientation.DIAGONAL, a: a, theta: theta, theta2: theta2
                };
            } else if (h) return {
        ...
        AnchorManager = function () {
            ...
            this.redraw = function (elementId, ui, timestamp, offsetToUI) {
                ...
                _updateOffset({
                    elId: elementId, offset: ui, recalc: false, timestamp: timestamp
                });
                ...
                for (var i = 0; i < endpointConnections.length; i++) {
                    ...
                    if (sourceContinuous || targetContinuous) {
                        ...
                        if (elementId != targetId) _updateOffset({
                            elId: targetId, timestamp: timestamp
                        });
                        if (elementId != sourceId) _updateOffset({
                            elId: sourceId, timestamp: timestamp
                        });
                ...
                for (var i = 0; i < ep.length; i++) {
                    ep[i].paint({
                        timestamp: timestamp, offset: myOffset, dimensions: myWH
                    });
                }
                ...
                if (fc)
                fc.paint({
                    timestamp: timestamp, recalc: false, elId: elementId
                });
                
                // paint all the connections
                for (var i = 0; i < connectionsToPaint.length; i++) {
                    connectionsToPaint[i].paint({
                        elId: elementId, timestamp: timestamp, recalc: false
                    });
                }
            };
        ...
        var Connection = function (params) {
            ...
            prepareEndpoint = function (existing, index, params, element, elementId, connectorPaintStyle, connectorHoverPaintStyle) {
                if (existing) {
                    ...
                    var ep = params.endpoints[index] || params.endpoint || _currentInstance.Defaults.Endpoints[index] || jsPlumb.Defaults.Endpoints[index] || _currentInstance.Defaults.Endpoint || jsPlumb.Defaults.Endpoint;
            ...
            this.moveParent = function (newParent) {
                ...
                // the Canvas implementation doesn't have to care about this
            };
            ...
            this.endpoints[0].paint({
                anchorLoc: anchorLoc, timestamp: initialTimestamp
            });
            ...
            this.paint = function (params) {
                ...
                var sourceInfo = _updateOffset({
                    elId: elId, offset: ui, recalc: recalc, timestamp: timestamp
                }),
                targetInfo = _updateOffset({
                    elId: tId, timestamp: timestamp
                });
            ...
            this.repaint = function (params) {
                ...
                this.paint({
                    elId: this.sourceId, recalc: recalc, timestamp: params.timestamp
                });
            };
        ...
        var _makeFloatingEndpoint = function (paintStyle, referenceAnchor, endpoint, referenceCanvas, sourceElement) {
            var floatingAnchor = new FloatingAnchor({
                reference: referenceAnchor, referenceCanvas: referenceCanvas
            });
            ...
            return _newEndpoint({
                paintStyle: paintStyle, endpoint: endpoint, anchor: floatingAnchor, source: sourceElement, scope: "__floating"
            });
        };
        ...
        var Endpoint = function (params) {
            ...
            _endpoint = new jsPlumb.Endpoints[renderMode][_endpoint](endpointArgs); else if (_endpoint. constructor == Array) {
            ...
            // the whole world.
            var argsForClone = jsPlumb.extend({
            },
            endpointArgs);
            ...
            this.connectorSelector = function () {
                ...
                if (self.isTarget && candidate) return candidate; else {
            ...
            this.paint = function (params) {
                ...
                if (! timestamp || self.timestamp !== timestamp) {
                    _updateOffset({
                        elId: _elementId, timestamp: timestamp, recalc: recalc
                    });
                    ...
                    if (xy) {
                        ...
                        if (ap == null) {
                            ...
                            if (xy == null || wh == null) {
                                _updateOffset({
                                    elId: _elementId, timestamp: timestamp
                                });
                                xy = offsets[_elementId];
                                wh = sizes[_elementId];
                            }
                            var anchorParams = {
                                xy:[xy.left, xy.top], wh: wh, element: self, timestamp: timestamp
                            };
            ...
            if (jsPlumb.CurrentLibrary.isDragSupported(_element)) {
                ...
                defaultOpts = jsPlumb.extend({
                },
                jpcl.defaultDragOptions),
                startEvent = jpcl.dragEvents[ "start"],
                stopEvent = jpcl.dragEvents[ "stop"],
                dragEvent = jpcl.dragEvents[ "drag"];
    ...
    jsPlumb.util = {
        ...
        pointOnLine: function (fromPoint, toPoint, distance) {
            ...
            return {
                x: fromPoint.x + x, y: fromPoint.y + y
            };
        },
        /**
...
(function () {
    ...
    jsPlumb.Connectors.Bezier = function (params) {
        ...
        this._findControlPoint = function (point, sourceAnchorPosition, targetAnchorPosition, sourceEndpoint, targetEndpoint, sourceAnchor, targetAnchor) {
            ...
            if (! perpendicular) {
                if (soo[0] == 0) // X
                p.push(sourceAnchorPosition[0] < targetAnchorPosition[0] ? point[0] + mi: point[0] - mi); else p.push(point[0] - (ma * soo[0]));
                
                if (soo[1] == 0) // Y
                p.push(sourceAnchorPosition[1] < targetAnchorPosition[1] ? point[1] + mi: point[1] - mi); else p.push(point[1] + (ma * too[1]));
            } else {
                if (too[0] == 0) // X
                p.push(targetAnchorPosition[0] < sourceAnchorPosition[0] ? point[0] + mi: point[0] - mi); else p.push(point[0] + (ma * too[0]));
    ...
    jsPlumb.Connectors.Flowchart = function (params) {
        ...
        findSegmentForLocation = function (location) {
            ...
            return {
                segment: segments[idx], proportion: inSegmentProportion, index: idx
            };
        };
    ...
    jsPlumb.Endpoints.Image = function (params) {
        ...
        this.compute = function (anchorPoint, orientation, endpointStyle, connectorPaintStyle) {
            ...
            widthToUse, heightToUse]; else return[0, 0, 0, 0];
        };
...
(function () {
    ...
    jsPlumb.Connectors.StateMachine = function (params) {
        ...
        this.gradientAtPoint = function (location) {
            if (isLoopback)
            return Math.atan(location * 2 * Math.PI); else
            return jsBezier.gradientAtPoint(_makeCurve(), location);
        };
    ...
    jsPlumb.Connectors.svg.StateMachine = function () {
        ...
        this.getPath = function (d) {
            ...
            if (d.length == 10)
            return "M " + d[4] + " " + d[5] + " C " + d[8] + " " + d[9] + " " + d[8] + " " + d[9] + " " + d[6] + " " + d[7]; else {
                // loopback
    ...
    jsPlumb.Connectors.vml.StateMachine = function () {
        ...
        this.getPath = function (d) {
            if (d.length == 10) {
                ...
                var o = "ar " + posString + "," + _conv(d[8]) + "," + _conv(d[9]) + "," + _conv(d[8]) + "," + _conv(d[9]) + " e";
...
(function () {
    ...
    _getGroup = function (container, connectorClass) {
        ...
        if (! g) {
            ...
            // todo if this gets reinstated, remember to use the current jsplumb instance.
    ...
    jsPlumb.Endpoints.vml.Dot = function () {
        ...
        this.getVml = function (d, atts, anchor) {
            return _node("oval", d, atts);
        };
    };
...
(function () {
    ...
    _updateGradient = function (parent, node, style, dimensions, uiComponent) {
        ...
        for (var i = 0; i < style.gradient.stops.length; i++) {
            ...
            styleToUse = dimensions[4] < dimensions[6] ? i: style.gradient.stops.length - 1 - i; else
            ...
            var s = _node(STOP, {
                "offset": Math.floor(style.gradient.stops[i][0] * 100) + "%", "stop-color": stopColor
            });
            g.appendChild(s);
        }
    ...
    _decodeFont = function (f) {
        ...
        var bits = f.match(r);
        return {
            size: bits[1] + bits[2], font: bits[3]
        };
    },
    ...
    var SvgConnector = jsPlumb.SvgConnector = function (params) {
        ...
        this._paint = function (d, style) {
            ...
            if (style.outlineColor) {
                var outlineWidth = style.outlineWidth || 1,
                outlineStrokeWidth = style.lineWidth + (2 * outlineWidth),
                outlineStyle = jsPlumb.CurrentLibrary.extend({
                },
                style);
...
(function ($) {
    ...
    jsPlumb.CurrentLibrary = {
        ...
        getDOMElement: function (el) {
            if (typeof (el) == "string") return document.getElementById(el); else if (el.context) return el[0]; else return el;
        },
        ...
        getUIPosition: function (eventArgs) {
            ...
            if (eventArgs.length == 1) {
                ret = {
                    left: eventArgs[0].pageX, top: eventArgs[0].pageY
                };
            } else {
...
(function () {
    if ("undefined" == typeof Math.sgn) Math.sgn = function (a) {
    ...
    w = Math.pow(2, -65), u = function (a, b) {
        for (var f =[], d = b.length -1, h = 2 * d -1, g =[], c =[], k =[], i =[], m =[[1, 0.6, 0.3, 0.1],[0.4, 0.6, 0.6, 0.4],[0.1, 0.3, 0.6, 1]], e = 0; e <= d; e++) g[e] = l.subtract(b[e], a);
        for (e = 0; e <= d -1; e++) c[e] = l.subtract(b[e +
        1], b[e]), c[e] = l.scale(c[e], 3);
        for (e = 0; e <= d -1; e++) for (var n = 0; n <= d; n++) k[e] ||(k[e] =[]), k[e][n] = l.dotProduct(c[e], g[n]);
        for (e = 0; e <= h; e++) i[e] ||(i[e] =[]), i[e].y = 0, i[e].x = parseFloat(e) / h; h = d -1; for (g = 0; g <= d + h; g++) {
            e = Math.max(0, g - h);
            for (c = Math.min(g, d);
            e <= c; e++) j = g - e, i[e + j].y += k[j][e] * m[j][e]
        }
        d = b.length -1; i = q(i, 2 * d -1, f, 0);
        h = l.subtract(a, b[0]);
        k = l.square(h);
        for (e = m = 0; e < i; e++) h = l.subtract(a, r(b, d, f[e], null, null)), h = l.square(h), h < k &&(k = h, m = f[e]);
        h = l.subtract(a, b[d]);
        h = l.square(h);
        h < k &&(k = h, m = 1);
        return {
    ...
    q = function (a, b, f, d) {
        var h =[], g =[], c =[], k =[], i = 0, m, e; e = Math.sgn(a[0].y);
        for (var n = 1; n <= b; n++) m = Math.sgn(a[n].y), m != e && i++, e = m; switch (i) {
            case 0: return 0; case 1: if (64 <= d) return f[0] =(a[0].x + a[b].x) / 2, 1; var o, i = a[0].y - a[b].y; m = a[b].x - a[0].x; e = a[0].x * a[b].y - a[b].x * a[0].y; n = max_distance_below = 0; for (o = 1; o < b; o++) {
                var l = i * a[o].x + m * a[o].y + e; l > n ? n = l: l < max_distance_below &&(max_distance_below = l)
            }
            o = m; n =(1 *(e - n) -0 * o) *(1 /(0 * o -1 * i));
            o = m; i =(1 *(e - max_distance_below) -0 * o) *(1 /(0 * o -1 * i));
            m = Math.min(n, i);
            if (Math.max(n,
            i) - m < w) return c = a[b].x - a[0].x, k = a[b].y - a[0].y, f[0] = 0 + 1 *(c *(a[0].y -0) - k *(a[0].x -0)) *(1 /(0 * c -1 * k)), 1
        }
        r(a, b, 0.5, h, g);
        a = q(h, b, c, d + 1);
        b = q(g, b, k, d + 1);
        for (d = 0; d < a; d++) f[d] = c[d]; for (d = 0; d < b; d++) f[d + a] = k[d]; return a + b
    },
    r = function (a, b, f, d, h) {
        for (var g =[[]], c = 0; c <= b; c++) g[0][c] = a[c]; for (a = 1; a <= b; a++) for (c = 0; c <= b - a; c++) g[a] ||(g[a] =[]), g[a][c] ||(g[a][c] = {
        }), g[a][c].x =(1 - f) * g[a -1][c].x + f * g[a -1][c + 1].x, g[a][c].y =(1 - f) * g[a -1][c].y + f * g[a -1][c + 1].y; if (null != d) for (c = 0; c <= b; c++) d[c] = g[c][0]; if (null != h) for (c = 0; c <= b; c++) h[c] =
        g[b - c][c]; return g[b][0]
    },
    v = {
    },
    x = function (a) {
    ...
    p = function (a, b) {
        for (var f = x(a.length -1), d = 0, h = 0, g = 0; g < a.length; g++) d += a[g].x * f[g](b), h += a[g].y * f[g](b);
        return {
            x: d, y: h
        }
    },
    s = function (a, b, f) {
        for (var d = p(a, b), h = 0, g = 0 < f ? 1: -1, c = null; h < Math.abs(f);) b += 0.005 * g, c = p(a, b), h += Math.sqrt(Math.pow(c.x - d.x, 2) + Math.pow(c.y - d.y, 2)), d = c; return {
            point: c, location: b
        }
    },
    t = function (a, b) {
        var f = p(a, b), d = p(a.slice(0, a.length -1), b), h = d.y - f.y, f = d.x - f.x; return 0 == h ? Infinity: Math.atan(h / f)
    };
    window.jsBezier = {
        distanceFromCurve: u, gradientAtPoint: t, gradientAtPointAlongCurveFrom: function (a,
        b, f) {
            b = s(a, b, f);
            if (1 < b.location) b.location = 1; if (0 > b.location) b.location = 0; return t(a, b.location)
        },
        nearestPointOnCurve: function (a, b) {
            var f = u(a, b);
            return {
                point: r(b, b.length -1, f.location, null, null), location: f.location
            }
        },
        pointOnCurve: p, pointAlongCurveFrom: function (a, b, f) {
            return s(a, b, f).point
        },
